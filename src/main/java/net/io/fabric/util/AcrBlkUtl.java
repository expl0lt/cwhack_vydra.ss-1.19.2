...
public enum AcrBlkUtl {;
   public static boolean isAnchorCharged(BlockPos anchor) {
      if (!isBlock(Blocks.RESPAWN_ANCHOR, anchor)) {
         ...
         try {
            return (Integer) BlockUtils2.getBlockState(anchor).get(RespawnAnchorBlock.CHARGES) != 0;
   ...
   public static boolean canPlace(BlockState state, BlockPos pos) {
      return antarctica.MC.world.canPlace(state, pos, (ShapeContext)null);
   }

   public static boolean hasBlock(BlockPos pos) {
      return !antarctica.MC.world.getBlockState(pos).isAir();
   }
   ...
   public static Block getBlock(BlockPos pos) {
      return antarctica.MC.world.getBlockState(pos).getBlock();
   }

   public static BlockState getBlockState(BlockPos pos) {
      return antarctica.MC.world.getBlockState(pos);
   }
   ...
   public static BlockHitResult clientRaycastBlock(BlockPos pos) {
      return antarctica.MC.world.raycastBlock(RotationUtils.getEyesPos(), RotationUtils.getClientLookVec().multiply(6.0D).add(RotationUtils.getEyesPos()), pos, getBlockState(pos).getOutlineShape(antarctica.MC.world, pos), getBlockState(pos));
   }
   ...
   public static boolean isBlockReachable(BlockPos pos, double reach) {
      ...
      VoxelShape shape = state.getOutlineShape(antarctica.MC.world, pos);
      if (shape.isEmpty()) {
         shape = fullCube();
      }
   ...
   public static boolean rightClickBlock(BlockPos pos) {
      ...
      if (shape.isEmpty()) {
         ...
         ActionResult result2 = antarctica.MC.interactionManager.interactBlock(antarctica.MC.player, Hand.MAIN_HAND, new BlockHitResult(hitVecs[side.ordinal()], side, pos, false));
